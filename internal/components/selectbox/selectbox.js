  ...
  function initSelect(wrapper) {
    ...
    newContent.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        e.preventDefault();
        const visibleItems = Array.from(newContent.querySelectorAll('.select-item')).filter(item =>
    ...
    const selectedItems = newContent.querySelectorAll('.select-item[data-selected="true"]');
      ...
      if (isMultiple) {
        if (showPills) {
          ...
          Array.from(selectedItems).forEach(selectedItem => {
            ...
            pill.className = 'flex items-center gap-1 px-2 py-1 text-xs rounded-md bg-primary text-primary-foreground';
            const pillText = document.createElement('span');
            pillText.textContent = selectedItem.querySelector('.select-item-text').textContent;
          ...
          setTimeout(() => {
            ...
            if (pillsWidth > valueWidth) {
              const selectedCountText = triggerButton.dataset.selectedCountText || `${selectedItems.length} items selected`;
              const msg = selectedCountText.replace('{n}', selectedItems.length);
    ...
    function selectItem(item) {
      if (!item || item.getAttribute('data-disabled') === 'true' || isSelecting) return;
