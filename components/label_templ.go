...
func LabelScript() templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var8 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 14, "\">\n\t\t\t// In LabelScript\n\t\t\tdocument.addEventListener('DOMContentLoaded', function() {\n\t\t\t  // Label-Klick-Handler - funktioniert automatisch für normale Inputs\n\t\t\t  // Für Buttons müssen wir nur einen Click-Handler hinzufügen\n\t\t\t  document.querySelectorAll('label[for]').forEach(function(label) {\n\t\t\t\tlabel.addEventListener('click', function(e) {\n\t\t\t\t  // Hole die for-ID direkt beim Klick, nicht bei der Initialisierung\n\t\t\t\t  const forId = this.getAttribute('for');\n\t\t\t\t  \n\t\t\t\t  // Finde das Zielelement mit der korrekten ID\n\t\t\t\t  const targetElement = document.getElementById(forId);\n\t\t\t\t  \n\t\t\t\t  // Wenn es ein Button ist, verarbeite den Klick\n\t\t\t\t  if (targetElement && targetElement.tagName === 'BUTTON') {\n\t\t\t\t\t// Verhindere das Standard-Verhalten des Labels\n\t\t\t\t\te.preventDefault();\n\t\t\t\t\t\n\t\t\t\t\t// Stoppe die Ereignispropagation, um doppelte Klicks zu vermeiden\n\t\t\t\t\te.stopPropagation();\n\t\t\t\t\t\n\t\t\t\t\tif (!targetElement.disabled) {\n\t\t\t\t\t  // Stelle sicher, dass der Fokus auf den Button gesetzt wird\n\t\t\t\t\t  targetElement.focus();\n\t\t\t\t\t  \n\t\t\t\t\t  // Verzögere den Klick leicht, um Event-Konflikte zu vermeiden\n\t\t\t\t\t  setTimeout(function() {\n\t\t\t\t\t\t// Erzeuge ein neues Klick-Ereignis für den Button\n\t\t\t\t\t\tconst clickEvent = new MouseEvent('click', {\n\t\t\t\t\t\t  bubbles: true,\n\t\t\t\t\t\t  cancelable: true,\n\t\t\t\t\t\t  view: window\n\t\t\t\t\t\t});\n\t\t\t\t\t\t\n\t\t\t\t\t\t// Löse das Ereignis auf dem Button aus\n\t\t\t\t\t\ttargetElement.dispatchEvent(clickEvent);\n\t\t\t\t\t  }, 10);\n\t\t\t\t\t}\n\t\t\t\t  }\n\t\t\t\t});\n\t\t\t\t\n\t\t\t\t// Disabled-Style-Handler\n\t\t\t\tfunction updateLabelStyle(label) {\n\t\t\t\t  const forId = label.getAttribute('for');\n\t\t\t\t  const targetElement = document.getElementById(forId);\n\t\t\t\t  \n\t\t\t\t  if (targetElement && targetElement.disabled) {\n\t\t\t\t\t// Hole den benutzerdefinierten Disabled-Style oder verwende Standard\n\t\t\t\t\tconst disabledStyle = label.getAttribute('data-disabled-style') || 'opacity-50 cursor-not-allowed';\n\t\t\t\t\tdisabledStyle.split(' ').forEach(function(className) {\n\t\t\t\t\t  if (className) {\n\t\t\t\t\t\tlabel.classList.add(className);\n\t\t\t\t\t  }\n\t\t\t\t\t});\n\t\t\t\t  } else {\n\t\t\t\t\t// Entferne den Disabled-Style\n\t\t\t\t\tconst disabledStyle = label.getAttribute('data-disabled-style') || 'opacity-50 cursor-not-allowed';\n\t\t\t\t\tdisabledStyle.split(' ').forEach(function(className) {\n\t\t\t\t\t  if (className) {\n\t\t\t\t\t\tlabel.classList.remove(className);\n\t\t\t\t\t  }\n\t\t\t\t\t});\n\t\t\t\t  }\n\t\t\t\t}\n\t\t\t\t\n\t\t\t\t// Initial Style aktualisieren\n\t\t\t\tupdateLabelStyle(label);\n\t\t\t\t\n\t\t\t\t// MutationObserver für dieses spezifische Label erstellen\n\t\t\t\tconst forId = label.getAttribute('for');\n\t\t\t\tconst targetElement = document.getElementById(forId);\n\t\t\t\t\n\t\t\t\tif (targetElement) {\n\t\t\t\t  const observer = new MutationObserver(function() {\n\t\t\t\t\tupdateLabelStyle(label);\n\t\t\t\t  });\n\t\t\t\t  \n\t\t\t\t  observer.observe(targetElement, { \n\t\t\t\t\tattributes: true, \n\t\t\t\t\tattributeFilter: ['disabled'] \n\t\t\t\t  });\n\t\t\t\t}\n\t\t\t  });\n\t\t\t});\n\t\t</script>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			return nil
